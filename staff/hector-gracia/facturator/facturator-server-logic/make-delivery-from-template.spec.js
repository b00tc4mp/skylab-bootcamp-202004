...
describe("makeDeliveryFromTemplate",()=>{
    ...
    it("should create a new delivery for a client getting the products and quantity from a template",async()=>{
        const results=await makeDeliveryFromTemplate(clientId,templateId)
    ...
    it("should return an error when given a client that does not exist",async()=>{
        const id=mongoose.ObjectId().toString()
        let error
        try {
            await makeDeliveryFromTemplate(id,templateId)
        } catch (_error) {
            error=_error
        }
    ...
    it("should return an error when given a template that does not exist",async()=>{
        ...
        let error
        try {
            await makeDeliveryFromTemplate(clientId,id)
        } catch (_error) {
            error=_error
        }
        expect(error).to.exist
