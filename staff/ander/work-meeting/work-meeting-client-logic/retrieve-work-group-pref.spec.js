...
const retrieveWorkGroupPref= require('./retrieve-work-group-pref')
const { random } = Math
...
describe('logic - retrieve solo work Group', () => {
    ...
    describe('when user already exists', async() => {
        ...
        it('should succeed on correct data', async () =>{
            debugger
            const result = await retrieveWorkGroupPref()

            expect(result).to.not.be.undefined
            expect(result.id.toString()).to.equal(workGroupId.toString())
        ...
        it('synchronous paths : should fail on a non-string token', () => {
            ...
            token = random()
            context.storage={token}
            expect(() => retrieveWorkGroupPref()).to.throw(TypeError, `${token} is not a string`)
            ...
            token = ''
            context.storage={token}
            expect(() => retrieveWorkGroupPref()).to.throw(Error, 'string is empty or blank')
