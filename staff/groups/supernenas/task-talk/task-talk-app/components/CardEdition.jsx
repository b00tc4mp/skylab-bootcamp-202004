...
function CardEdition({onCreate,onUpdate, onDelete, editCard}) {
    ...
    const handleChangeTitle = (event) => setCardTitle(event.target.value)

    const handleChangeMessage = (event) => setCardMessage(event.target.value)
    
    const handleOnDelete = (event) => {
        event.preventDefault()
        
        if(editCard) onDelete(editCard.id)    
    }
