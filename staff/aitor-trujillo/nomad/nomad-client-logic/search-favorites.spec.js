...
const searchFavorites = require('./search-favorites')
const { random } = Math
...
describe('logic - retrieve workspace by id', () => {
    ...
    it('should succeed on valid workspaceId', async () => {
        const result = await searchFavorites('name')
    ...
    it('should fail on no found result', async () => {
        const result = await searchFavorites(random().toString())
            .then(() => { throw new Error('should not reach this point') })
            .catch(error => {
                expect(error).to.be.an.instanceof(Error)
                expect(error.message).to.equal('No results for your query :(')
            })
