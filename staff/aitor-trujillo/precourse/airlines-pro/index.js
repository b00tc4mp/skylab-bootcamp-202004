...
function removeFlight() {
    ...
    var findFlight = flights.indexOf(
        flights.find(x => x.id === removeId)
    );
    // Eliminar y mostrar
...
function userChoice() {
    var choice = parseInt(
        prompt('Introduce el ID para comprar el billete.')
    );
    var selectFlight = flights.indexOf(
        flights.find(x => x.id === choice)
    );
    if (selectFlight != -1) {
