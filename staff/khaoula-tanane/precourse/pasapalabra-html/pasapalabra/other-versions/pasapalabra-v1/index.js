  ...
  [
    {
      ...
      status: 0,
      question:
        "CON LA A. Instrumento o máquina que sirve para atacar o defenderse: Apoderarse de alguien"
    },
    {
      letter: "b",
      answer: "bañarse",
    ...
    },
    {
      ...
      status: 0,
      question:
        "CONTIENE LA Ñ. Objeto que se utiliza para atraer a las aves que se quieren cazar"
    },
    {
      letter: "o",
      answer: "oveja",
    ...
    },
    {
      ...
      status: 0,
      question:
        "CON LA Q. Tener el deseo, la voluntad o la intención de hacer, poseer o lograr algo"
    },
    {
      letter: "r",
      answer: "resumen",
    ...
    },
    {
      ...
      status: 0,
      question:
        "CON LA W. Bebida alcohólica de alta graduación que se obtiene por destilación de cebada y otros cereales"
    },
    {
      letter: "x",
      answer: "xilofono",
...
questionsSource.forEach(block => {
  block.forEach((question, i) => {
    if (Math.random() >= 0.5) {
      questions[i] = questionsSource[0][i]
    } else {
      questions[i] = questionsSource[1][i]
    }
  })
})
...
const arrayOfLists = Array.from(lists);
...
function play() {
  ...
  questionDiv.innerText = questions[position].question;

  lists[position].classList.add("selected");

  let modal = document.getElementById("startGame");
...
pasapalabra.addEventListener('click', clearNext);
function clearNext(){
  var input = document.getElementById('userAnswer')
  input.value = ''
  next()
...
function next() {
  ...
  time = 11;
  if (secondIteration && position >= questions.length) {
    return console.log("Game over");
  }
  //eliminar los estilos una vez no esté seleccionado
  arrayOfLists[position].classList.remove("selected");

  var inputAnswer = document.getElementById("userAnswer");
  var userAnswer = inputAnswer.value.toLowerCase();

  if (userAnswer === questions[position].answer) {
    console.log("respuesta correcta");
    arrayOfLists[position].classList.add("correct");
    questions[position].status = 1;
  } else if (userAnswer === "" || userAnswer === "pasapalabra") {
    console.log("saltar pregunta");
    //a  questions[position].status = 0
  } else {
    arrayOfLists[position].classList.add("wrong");
  ...
  if (secondIteration && position >= questions.length) {
    var correct = questions.filter(question => question.status === 1);
    console.log(correct.length, "respuestas correctas");

    var incorrect = questions.filter(question => question.status === -1);
    console.log(incorrect.length, "respuestas incorrectas");

    var saltadas = questions.filter(question => question.status === 0);
    console.log(saltadas.length, "preguntas saltadas");
  ...
  if (questions[position].status !== 0) {
    return next();
  }
