function pasapalabraGame() {
    ...
    function setUserName() {
      name = element('#userName').value.toLowerCase();
    }
    

    function fillQuestions() {

        questionsSource.forEach(block => {
            block.forEach((question, i) => {
              questions[i] = (Math.random() >= 0.5) ? questionsSource[0][i] : questionsSource[1][i]
            })
          })
    }
    ...
    function drawCircle() {
      questions.forEach((q, i) => {
        ...
        lists[i].style.transform = `rotate(${rotate}deg) translate(20em) rotate(${rotate * (-1)}deg)`;
      });
    }
    ...
    function timer(){
        time -= 1
        element('.rightSide__time').innerText = time;
        if(time <= 0){
            time = 10
            next()
        }
    ...
    function callTimer() {
      ...
      function replay(){
        ...
        element('.leftSide__question__result').innerText = ''; 
        lists.forEach(list =>{
          list.className = ""
        })
        play()
      ...
      function printQuestion() {
        element('.leftSide__question__text').innerText = questions[position].question;
      }
    
      function markAsSelected() {
          lists[position].classList.add("selected");
      }
    
      function hideModal() {
          element(".leftSide__modal").classList.add("leftSide__modal--hide");
      }
    
      function callNextOnEnter() {
          element('#userAnswer').addEventListener('keyup',(event) => {
            if (event.keyCode === 13) {
            next()
          }
          })
     ...
     function startGame() {
         element('#start').addEventListener('click', play);
     } 
    
     function handleClearNext() {
         element('#pasapalabra').addEventListener('click', clearNext);
     }
      ...
      function removeSelected() {
        lists[position].classList.remove("selected");    
      }
    
      function getUserAnswer() {
        return element('#userAnswer').value.toLowerCase();
      }
      ...
      function setStatus(status) {
          questions[position].status = status;
      }
    
      function markAsCorrect() {
        lists[position].classList.add("correct");
      }
    
      function markAsWrong() {
          lists[position].classList.add("wrong");
      }
      
      function validateAnswer() {
        let userAnswer = getUserAnswer()
    
        if (userAnswer === questions[position].answer) {
            markAsCorrect()
            setStatus(1)
      ...
      function getRankingFromStorage() {
        ...
        let correct = filterByStatus(1).length;
        let incorrect = filterByStatus(-1).length;
        let skipped = filterByStatus(0).length;

        let userData  = {name, correct, incorrect, skipped};
      ...
      function saveOnStorage(newRanking){
          localStorage.setItem('ranking', JSON.stringify(newRanking));
      }

      function sortRankings(newRanking) {
        newRanking.sort((a, b) => {
      ...
      function printTemplate(newRanking) {

        element('.leftSide__question__result').innerHTML = '<button class="replay" onclick="pasapalabra.replay()" >Reiniciar</button>';

        newRanking.forEach(rank => {
          element('.leftSide__question__result').innerHTML += `
              <div class='ranking'>
              <span class='ranking_name'> ${rank.name.slice(0,12)}</span>
              <span class='rank_status ranking_correct'> ${rank.correct}</span>
              <span class= 'rank_status ranking_incorrect'>${rank.incorrect}</span>
              <span class='rank_status ranking_pasapalabra'>${rank.skipped}</span>
              </div>
          `;
        })
      }
      ...
      function printResults() {
        ...
        let newRanking = ranking.slice(ranking.length-5)
        
        sortRankings(newRanking) 
        
        saveOnStorage(newRanking)

        printTemplate(newRanking)
    
        clearInterval(timerInterval);
      }
    
      function setSecondIteration() {                 
        secondIteration = true; 
        position = 0;
      }
    
      function isItOver() {
       return (secondIteration && position >= questions.length)
      }
    ...
    function next() {
        ...
        if (isItOver()) return console.log("Game over");
        ...
        if (isItOver()) return printResults()
        
        const isRoundComplete =  position >= questions.length;
        if (isRoundComplete) setSecondIteration()
        
        const isItAnswered = questions[position].status !== 0;
        if (isItAnswered) return next();
    ...
    return { init, next, replay }
}

const pasapalabra = pasapalabraGame()
pasapalabra.init()