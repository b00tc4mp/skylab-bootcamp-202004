...
function calculator() {
    ...
    if (arg.length > 1) {
        ...
        oper = decimals(oper);
        lista = ["Suma " + ": " + oper[0], "Resta " + ": " + oper[1], "Multiplicación " + ": " + oper[2], "División " + ": " + oper[3]];
        console.log(lista);
    } else {
        oper = [Math.sqrt(arg[0])];
        oper = decimals(oper);
        lista = ["Raíz cuadrada " + ": " + oper[0]];
        console.log(lista);
    }
    let another = prompt("New numbers? y/n");
    while (true) {
        if (another == "y") {
            calculator();
            break;
            ...
            another = prompt("New numbers? y/n");
        }
    }
}
