...
const stanzaParser = (stanza) => {
  return stanza
    ...
    .reduce( (prev, curr) => {
      ...
      if(key === "id") prev["HPO_id"] = val
      else if(key === "is_a"){
        if (!prev[key]) prev[key] = []

        prev[key].push(val.split(' ! ')[0])
      }
      else if(key === "synonym"){
        if (!prev[key]) prev[key] = []

        prev[key].push(val.split("\"")[1])
      }
      else if(key === "xref"){
        if (!prev[key]) prev[key] = []
        ...
        prev[key] = val.split("\"")[1]
      }
      else if (key === "name" || key === "def" || key === "comment" || key ==="alt_id"){
        prev[key] = val
      }
      else if (key === "created_by" || key === "creation_date" || key === "comment" || key === "" || key === "property_value"){
      }
      else prev[key] = val
