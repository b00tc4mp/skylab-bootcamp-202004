//Función que define el array de preguntas
...
//Definimos una función que sea un ciclo de preguntas
function makeAQuestion(multipleQuestions, questionsStatus, points, mistakes){
    //Pedimos la respuesta
    answer = prompt(multipleQuestions[i][(questionsStatus[i].question - 1)].question + '\nIntroduce END para salir del juego.');
    //Si la respuesta es igual a la de la base de datos, cambiamos el estatus a correcto y añadimos un punto
    if (answer.toLowerCase() == multipleQuestions[i][(questionsStatus[i].question - 1)].answer){
        questionsStatus[i].status = 1;
        points = points + 1;
        confirm('CORRECTO!');
    }
    //Si la respuesta es pasapalabra, no cambiamos nada
    else if (answer.toLowerCase() == 'pasapalabra'){
        confirm('PASAPALABRA!');
    }
    //Si la respuesta es 'END', salimos del juego y decios cuantos puntos se han conseguido
    else if (answer.toLowerCase() == 'end'){
        confirm(`Has conseguido ${points} puntos. Gracias por participar!`);
        return;
    }
    //Si la respuesta es incorrecta, cambiamos el estatus a incorrecto y añadimos un error
    else{
        questionsStatus[i].status = 2;
        mistakes = mistakes + 1;
        confirm('RESPUESTA INCORRECTA!');
    };
};

function cycleOfQuestions(multipleQuestions){
    points = 0;
    mistakes = 0;
    //Hago una variable del status de las preguntas dentro de la función para que no cambie el status del listado 
    //de preguntas original.
    //El key 'question' nos dirá qué pregunta hemos hecho dentro del listado original (escogida aleatoriament)
    //El key 'status' hará el papel del 'status' del array de preguntas original.
    var questionsStatus = [
        {letter: 'a', status: 0, question: 0},
        {letter: 'b', status: 0, question: 0},
        {letter: 'c', status: 0, question: 0},
        {letter: 'd', status: 0, question: 0},
        {letter: 'e', status: 0, question: 0},
        {letter: 'f', status: 0, question: 0},
        {letter: 'g', status: 0, question: 0},
        {letter: 'h', status: 0, question: 0},
        {letter: 'i', status: 0, question: 0},
        {letter: 'j', status: 0, question: 0},
        {letter: 'k', status: 0, question: 0},
        {letter: 'l', status: 0, question: 0},
        {letter: 'm', status: 0, question: 0},
        {letter: 'n', status: 0, question: 0},
        {letter: 'ñ', status: 0, question: 0},
        {letter: 'o', status: 0, question: 0},
        {letter: 'p', status: 0, question: 0},
        {letter: 'q', status: 0, question: 0},
        {letter: 'r', status: 0, question: 0},
        {letter: 's', status: 0, question: 0},
        {letter: 't', status: 0, question: 0},
        {letter: 'u', status: 0, question: 0},
        {letter: 'v', status: 0, question: 0},
        {letter: 'w', status: 0, question: 0},
        {letter: 'x', status: 0, question: 0},
        {letter: 'y', status: 0, question: 0},
        {letter: 'z', status: 0, question: 0}
    ]
    do{
        for (i = 0; i < multipleQuestions.length; i++){
            //Si la pregunta no ha sido respondida y no se ha definido la pregunta aleatoriamente para esta letra
            if (questionsStatus[i].status == 0 && questionsStatus[i].question == 0){
                //Definimos la pregunta de forma aleatoria
                questionsStatus[i].question = Math.round(Math.random() * (multipleQuestions[i].length - 1)) + 1;
                //Y preguntamos
                //Pedimos la respuesta
                answer = prompt(multipleQuestions[i][(questionsStatus[i].question - 1)].question + '\nIntroduce END para salir del juego.');
                //Si la respuesta es igual a la de la base de datos, cambiamos el estatus a correcto y añadimos un punto
                ...
                //Si la respuesta es 'END', salimos del juego y decios cuantos puntos se han conseguido
                    ...
                    confirm(`Has salido de la partida con ${points} aciertos y ${mistakes} errores. Gracias por participar!`);
                    return;
                }
                //Si la respuesta es incorrecta, cambiamos el estatus a incorrecto y añadimos un error
                else{
            ...
            //Si la pregunta ya se ha definido, solamente hacemos la pregunta
            else if(questionsStatus[i].status == 0){
                //Pedimos la respuesta
                answer = prompt(multipleQuestions[i][(questionsStatus[i].question - 1)].question + '\nIntroduce END para salir del juego.');
                //Si la respuesta es igual a la de la base de datos, cambiamos el estatus a correcto y añadimos un punto
                if (answer.toLowerCase() == multipleQuestions[i][(questionsStatus[i].question - 1)].answer){
            ...
            //Si la preguna ya ha sido respondida, no se ejecuta el código y pasamos a la siguiente pregunta
        };
    } while (points + mistakes !== multipleQuestions.length);
};
...
function updateScoreboard(playerName, points, scoreboard){
    ...
    for (i = 0; i < scoreboard.length; i++){
        scoreboardString += `\nJugador ${i + 1}: ${scoreboard[i].name}, ${scoreboard[i].points} preguntas acertadas`;
    };
    alert(scoreboardString);
};

// esquema básico: confirm que empieza el juego y empezamos los ciclos de preguntas hasta que el total de 
// respuestas correctas e incorrectas sea igual al total de preguntas.
...
function pasapalabra(){
    ...
    //creaos el scoreboard
    var scoreboard = [];
    //Defnimos las preguntas
    var multipleQuestions = defineQuestions();
    do{
        //Ejecutamos los ciclos de preguntas
        cycleOfQuestions(multipleQuestions);
        //Si se ha introducido end, cerramos antes de que devuelva el mensaje final
        if (points + mistakes !== multipleQuestions.length){
            return;
        };
        //Mensaje final
        confirm(`Felicidades, has completado el juego con ${points} palabras acertadas y ${mistakes} errores!`);
        //Actualizamos el scoreboard y lo mostramos
