...
let rankingUsers = {
    userPlayers: [],
    addPlayer: function(playerName, correctWords) {
        let userPlayer = {
            player: playerName,
            correctWords: correctWords,
        };
        this.userPlayers.push(userPlayer);
        return this.userPlayers;
    }, 

    printRanking: function() {
        this.userPlayers.sort(function (player1, player2) {
           return player2.correctWords - player1.correctWords;
          });
          console.log('RANKING DE JUGADORES\n\tJugador\t\tLetras acertadas');
          for (let i = 0; i < this.userPlayers.length; i++){
              console.log(`${i + 1}.\t${this.userPlayers[i].player}\t\t${this.userPlayers[i].correctWords}`);
        }
...
function calculateWordPosition(wordPosition) {
    if (wordPosition === multiplesQuestions.length - 1) { 
        wordPosition = 0;
    } else {
        wordPosition++;
    }
...
function getPlayerPoints(){
    let playerPoints = 0;
    for (let i = 0; i< multiplesQuestions.length; i++) {
      if (multiplesQuestions[i][answerPosition].status === 1) {
        playerPoints++;
      } 
    }
    return playerPoints;
...
function modifyStatus(userAnswer, word) {
    if (userAnswer.localeCompare(word.answer, 'es', { sensitivity: 'base' }) === 0) {
        word.status = 1;
        let pointsPlayer = getPlayerPoints();
        alert(`La respuesta ${word.answer} es correcta!\nTienes ${pointsPlayer} puntos.`);
    }
        ...
        word.status = -1;
        alert(`La respuesta '${userAnswer}' es incorrecta!\nLa respuesta correcta es: ${word.answer}.`);
    }
}

function nextWord(wordPosition) {
    if (checkQuestionsCompleted()) {
        let correctWords = getPlayerPoints();
        let incorrectWords = multiplesQuestions.length - correctWords;
        alert(`Felicidades, has completado el pasapalabras!\nHas acertado ${correctWords} letras.\nHas fallado ${incorrectWords} letras.`);
        rankingUsers.addPlayer(playerName, correctWords);
        rankingUsers.printRanking();
    } else {
        let word = multiplesQuestions[wordPosition][answerPosition];
        
        if (word.status === 0 ) {
            let userAnswer = prompt(word.question);
    
            if (userAnswer === null || userAnswer.toLowerCase() === 'end') {
                let correctWords = getPlayerPoints();
                alert(`Has acertado ${correctWords} letras.\nAdios!`);
                return;
            } 
            modifyStatus(userAnswer, word); 
        }
        nextWord(calculateWordPosition(wordPosition));
    }
}
...
function pasaPalabra() {
    ...
    do {
        ...
        if (playerName === null) {
            ...
            alert(`Hola ${playerName}, bienvenido/a! a pasaPalabra!`);
        }
